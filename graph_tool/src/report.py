...
def procedure(g):
    with open("../report/" + g.gp["name"] + "-report.txt", "a+") as file:
        ...
        while g.num_vertices() > 0:
            ...
            file.write("numero di vertici di taglio: " + str(len(list(filter(lambda x: (x == 1), cv)))) + "\n")
            # per ogni componente connessa
            for i in range(len(hist)):
                file.write("\t------ analisi della componente connessa #" + str(i) + " ------\n")
                ...
                # calcolo bctree
                bct = bc.BCTree(sub_g, adj_matrix, cv)
